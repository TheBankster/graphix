...
from modeler import RenderThreats
...
#       - Control type
#       - Satisfaction indicator (boolean)
...
def ExternalActorInternalSystem() -> Set[Tuple[STRIDE, FrozenSet[Tuple[str, str, str, CONTROL, bool]]]]:
    query: str = """
        ...
        WHERE {
            ...
            OPTIONAL { 
                ?interface :requiresControl :ClientAuthentication .
                BIND(true AS ?hasClientAuth)
            }
            OPTIONAL { 
                ?interface :requiresControl :RateLimiting . 
                BIND(true AS ?hasRateLimit) 
            }
            OPTIONAL { 
                ?interface :requiresControl :TrafficEncryption . 
                BIND(true AS ?hasTransitEnc) 
            }

            # Check internal system for logic/authorization controls
            OPTIONAL { 
                ?system :requiresControl :AccessControl . 
    ...
    for result in bindings:
        ...
        results.add((
            ...
            frozenset([(actor_uri, system_uri, interface_uri, CONTROL.TRAFFIC_ENCRYPTION, "hasTransitEnc" in result)])
        ))
...
def InternalSystemExternalSystem() -> Set[Tuple[STRIDE, FrozenSet[Tuple[str, str, str, CONTROL, bool]]]]:
    ...
    query: str = """
        ...
        WHERE {
            ...
            OPTIONAL { 
                ?internalSystem :requiresControl :ServerAuthentication . 
                BIND(true AS ?hasAuth)
            }
            OPTIONAL { 
                ?internalSystem :requiresControl :TrafficEncryption . 
    ...
    for result in bindings:
        ...
        results.add((
            STRIDE.INFORMATION_DISCLOSURE, 
            frozenset([(external_system_uri, internal_system_uri, internal_system_uri, CONTROL.TRAFFIC_ENCRYPTION, "hasEncryption" in result)])
        ))
